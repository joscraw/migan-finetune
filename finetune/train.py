...
Uses MI-GAN's original adversarial training (keeps output sharp) PLUS
identity loss and boundary loss (fixes the visible box artifact).
"""
...
from finetune.losses import (
    ...
    PatchDiscriminator, discriminator_loss, generator_adversarial_loss,
...
def save_comparison(model, img, mask, save_path, device):
    ...
    total_w = sum(im.width for im in images) + 20
    h = images[0].height
...
def train(
    ...
    """
    Fine-tune MI-GAN with adversarial + identity + boundary losses.

    Adversarial loss keeps the output sharp (same principle as original MI-GAN).
    Identity + boundary losses fix the box artifact.
    """
    ...
    disc = PatchDiscriminator(in_channels=4, base_channels=64).to(device)
    disc.train()
    disc_param_count = sum(p.numel() for p in disc.parameters())
    print(f"[Discriminator] PatchGAN initialized — {disc_param_count:,} parameters")
    ...
    # Cosine decay for generator
    g_scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(g_optimizer, T_max=num_steps, eta_min=g_lr * 0.01)
    ...
    # Fixed test sample for visualization
    test_img, test_mask = dataset[0]

    print(f"\n{'='*60}")
    print(f"MI-GAN Fine-Tuning (with adversarial loss)")
    print(f"{'='*60}")
    print(f"Steps: {num_steps}")
    print(f"Batch size: {batch_size}")
    print(f"G lr: {g_lr} | D lr: {d_lr}")
    ...
    print(f"R1 penalty: gamma={r1_gamma}, every {d_reg_every} steps")
    ...
    for step in range(1, num_steps + 1):
        ...
        # Model input: [mask-0.5, image*mask]
        ...
        # Adversarial loss (keep it sharp)
        ...
        d_loss = discriminator_loss(disc, images, fake_output, masks)

        # R1 regularization (every N steps)
        d_reg = torch.tensor(0.0, device=device)
        if step % d_reg_every == 0:
            d_reg = r1_penalty(disc, images, masks) * r1_gamma * 0.5
            d_loss = d_loss + d_reg
        ...
        if step % log_every == 0 or step == 1:
            ...
            print(f"[Step {step:>6d}/{num_steps}] "
                  ...
                  f"adv: {l_adv.item():.4f} | "
                  f"percep: {l_percep_val:.4f} | "
                  f"d_loss: {d_loss.item():.4f} | "
                  f"lr: {g_scheduler.get_last_lr()[0]:.2e} | "
                  f"ETA: {eta/60:.1f}min")
    ...
    # Save final model (generator only — this is what gets converted to Core ML)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--output', type=str, default='./finetune_output')
    ...
    train(model=model, dataset=dataset, output_dir=args.output,
          num_steps=args.steps, batch_size=args.batch_size, device=device)
